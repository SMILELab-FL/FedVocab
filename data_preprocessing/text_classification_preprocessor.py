...
class TLMPreprocessor(BasePreprocessor):
    ...
    def transform(self, X, y, index_list=None, evaluate=False, idx=-1):
        # index_list is creat for setting guid
        if index_list is None:
            index_list = [i for i in range(len(X))]
        examples = self.transform_examples(X, y, index_list)
        features = self.transform_features(examples, evaluate=evaluate, idx=idx)

        all_guid = torch.tensor([int(f.guid) for f in features], dtype=torch.long)
    ...
    def transform_features(self, examples, evaluate=False, no_cache=False, silent=False, idx=-1):
        """
        Converts a list of InputExample objects to a TensorDataset containing InputFeatures. Caches the InputFeatures.
        """
        process_count = self.args.process_count
        randomEmbedder = registry.get("randomEmbedder", None)
        other_embedder_ids = registry.get("other_embedder_ids", None)

        if randomEmbedder and idx != -1:
            if idx in other_embedder_ids:
                tokenizer = registry.get("other_tokenizer")
            else:
