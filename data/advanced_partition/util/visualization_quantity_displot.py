...
quantity_partition_methods = ['niid_quantity_beta=100.0', 'niid_quantity_beta=10.0','niid_quantity_beta=5.0','niid_quantity_beta=1.0']
...
for index, partition in enumerate(quantity_partition_methods):
    ...
    for client_id in partition_file[partition_data_path].keys():
        client_len = len(partition_file[partition_data_path][client_id]['train'][()]) + len(partition_file[partition_data_path][client_id]['test'][()])
