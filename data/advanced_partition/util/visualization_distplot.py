...
print("tc_partition_methods: ", tc_partition_methods)
...
if args.task_type == "text_classification":
    ...
    client_assignment = [decode_data_from_h5(data['Y'][i][()]) for i in data['Y'].keys()]
...
for index, partition in enumerate(tc_partition_methods):
    ...
    client_sum_order = np.append(client_sum_order,np.array([sum(i) for i in heat_map_data]), axis=0)
...
g = sns.displot(df,x = "label_sum",hue="alpha",kind="kde",common_norm=False, fill=True)
